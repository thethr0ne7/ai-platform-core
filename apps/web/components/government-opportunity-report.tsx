...
function displayValue(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    ...
    return text(object.label, text(object.name, text(object.value, JSON.stringify(object))));
  }
  return "—";
}
...
export function GovernmentOpportunityReport({ report }: { report: GovernmentOpportunityReportData }) {
  ...
  const matches = report.measure_matches?.length ? report.measure_matches : report.support_measures ?? [];
  ...
  const bestMatch = Math.round(numberValue(readiness.best_match_score));
    ...
    <section className="mt-6 space-y-4 pb-28">
      <article className="glass-surface relative overflow-hidden rounded-[30px] p-5 sm:p-7">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-center">
          <div>
            <div className="status-pill"><Gauge size={15} /> Готовность проекта</div>
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-mist/55">
              <span>Текущая готовность</span><span className="font-semibold text-signal">{score}%</span>
            </div>
          </div>
          <div className="clay-inset grid min-h-44 place-items-center rounded-[28px] p-5 text-center">
            <div>
              ...
              <p className="mt-4 text-xs leading-5 text-mist/45">Оценка учитывает документы, землю, форму заявителя, факты, меры поддержки и источники.</p>
            </div>
          </div>
        </div>
      </article>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric icon={<FileCheck2 size={18} />} title="Документы" value={`${numberValue(documents.parsed)} из ${numberValue(documents.total)}`} note="разобрано системой" />
        ...
        <Metric icon={<Landmark size={18} />} title="Подходящие варианты" value={String(matches.length)} note={bestMatch ? `лучшее совпадение ${bestMatch}%` : "ожидают уточнения"} />
        <Metric icon={<ShieldCheck size={18} />} title="Доказательная база" value={String(numberValue(evidence.evidence_records))} note={`${verificationRate}% подтверждено`} />
      </div>

      <ReportBlock title="Что уже подключено" subtitle="Возможности, которые участвуют в текущем анализе" icon={<Sparkles size={19} />}>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {capabilities.length ? capabilities.map((item, index) => (
            <div key={`${text(item.name)}-${index}`} className="clay-inset rounded-[22px] p-4">
              <div className="flex items-start justify-between gap-3"><p className="font-medium leading-5">{text(item.name)}</p><span className="signal-dot mt-1 shrink-0" /></div>
              <p className="mt-3 text-xs leading-5 text-mist/55">{text(item.detail)}</p>
              <p className="mt-3 text-[11px] font-medium text-signal">{label(item.status)}</p>
            </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <ReportBlock title="Исходные данные проекта" subtitle="Что система уже знает и чего не хватает" icon={<BookOpenCheck size={19} />}>
          ...
          {report.blockers?.length ? <div className="mt-4 rounded-[22px] border border-signal/20 bg-signal/[.045] p-4"><div className="flex items-center gap-2"><AlertCircle className="text-signal" size={18} /><h3 className="font-semibold">Что сейчас мешает</h3></div><ul className="mt-3 space-y-2 text-sm leading-6 text-mist/65">{report.blockers.map((item) => <li key={item}>• {item}</li>)}</ul></div> : null}
        </ReportBlock>
      </div>

      <ReportBlock title="Подходящие меры поддержки" subtitle="Сопоставление проекта с требованиями программ" icon={<Landmark size={19} />}>
        <div className="grid gap-3 lg:grid-cols-2">
          {matches.length ? matches.map((measure, index) => {
            ...
            return <article key={String(measure.id ?? `${text(measure.title)}-${index}`)} className="clay-inset rounded-[24px] p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-signal/20 bg-signal/[.05] px-3 py-1.5 text-signal">{label(measure.eligibility_status, "Проверяется")}</span><span className="rounded-full border border-white/10 px-3 py-1.5 text-mist/55">{label(measure.status)}</span></div>
              {blockers.length ? <MiniList title="Что мешает" items={blockers} /> : null}
              {missing.length ? <MiniList title="Что нужно уточнить" items={missing} /> : null}
          ...
          }) : <Empty value="Система не нашла подтверждённых совпадений. Добавьте документы и уточните форму заявителя." />}
        </div>
      </ReportBlock>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <ReportBlock title="Документы проекта" subtitle="Загрузка и фактический разбор файлов" icon={<FileText size={19} />}>
          ...
          {documents.categories?.length ? <div className="mt-4 space-y-2">{documents.categories.map((category, index) => <div key={`${category.name}-${index}`} className="flex items-center justify-between rounded-2xl border border-white/[.07] px-4 py-3 text-sm"><span className="text-mist/65">{text(category.name)}</span><span className="font-semibold text-signal">{numberValue(category.count)}</span></div>)}</div> : null}
        </ReportBlock>

        <ReportBlock title="Факты о проекте" subtitle="Данные из анкеты, документов и выводов системы" icon={<ScanSearch size={19} />}>
          <div className="space-y-2">
            {facts.length ? facts.slice(0, 12).map((fact, index) => <div key={`${text(fact.code)}-${index}`} className="clay-inset rounded-[20px] p-4"><div className="flex items-start justify-between gap-3"><p className="text-sm font-medium">{label(fact.code, `Факт ${index + 1}`)}</p><span className="text-[10px] text-signal">{label(fact.status)}</span></div><p className="mt-2 break-words text-sm leading-6 text-mist/60">{displayValue(fact.value)}</p><p className="mt-2 text-[10px] text-mist/35">Источник: {label(fact.source)} · уверенность {Math.round(numberValue(fact.confidence) * (numberValue(fact.confidence) <= 1 ? 100 : 1))}%</p></div>) : <Empty value="Факты появятся после заполнения проекта и разбора документов." />}
          </div>
        </ReportBlock>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <ReportBlock title="Новые изменения" subtitle="Что изменилось в официальных источниках" icon={<GitCompareArrows size={19} />}>
          <div className="space-y-3">
            {changes.length ? changes.map((change, index) => <article key={String(change.id ?? index)} className="clay-inset rounded-[22px] p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] text-signal">{label(change.change_type)}</p><h3 className="mt-2 font-semibold leading-6">{text(change.document_title, "Изменение в официальном документе")}</h3></div><span className="text-[10px] text-mist/40">{dateLabel(change.detected_at)}</span></div><p className="mt-3 text-sm leading-6 text-mist/60">{text(change.summary)}</p><p className="mt-3 text-xs text-mist/40">{text(change.authority)} · {text(change.source_name)}</p>{typeof change.document_url === "string" && change.document_url && <a className="mt-3 inline-flex items-center gap-1 text-sm text-signal" href={change.document_url} target="_blank" rel="noreferrer">Открыть документ <ArrowUpRight size={14} /></a>}</article>) : <Empty value="Новых подтверждённых изменений для проекта пока нет." />}
          </div>
        </ReportBlock>

        <ReportBlock title="Ранние сигналы" subtitle="Направления, которые могут повлиять на проект" icon={<Radar size={19} />}>
          <div className="space-y-3">
            {signals.length ? signals.map((signal, index) => { const confidence = Math.round(numberValue(signal.confidence) * (numberValue(signal.confidence) <= 1 ? 100 : 1)); return <article key={String(signal.id ?? index)} className="clay-inset rounded-[22px] p-4"><div className="flex items-start justify-between gap-3"><p className="text-[11px] text-signal">{label(signal.type)}</p><span className="text-xs font-semibold text-signal">{confidence}%</span></div><h3 className="mt-2 font-semibold leading-6">{text(signal.title)}</h3><p className="mt-2 text-sm leading-6 text-mist/60">{text(signal.summary)}</p><p className="mt-3 text-xs text-mist/40">Горизонт: {numberValue(signal.horizon_months)} мес. · {label(signal.level)}</p></article>; }) : <Empty value="Сигналы собираются, но для текущего профиля пока нет точного совпадения." />}
          </div>
        </ReportBlock>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <ReportBlock title="Доказательная база" subtitle="Насколько выводы привязаны к официальным документам" icon={<ShieldCheck size={19} />}>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <SmallMetric title="Официальные источники" value={numberValue(evidence.official_sources)} accent />
            ...
            <SmallMetric title="Подтверждено" value={numberValue(evidence.verified_records)} accent />
            <SmallMetric title="Доля проверки" value={`${verificationRate}%`} />
          </div>
          <p className="mt-4 rounded-2xl border border-signal/20 bg-signal/[.045] p-4 text-sm leading-6 text-mist/65">Правило: {text(evidence.policy, "Только официальные источники")}</p>
        </ReportBlock>
      ...
      <ReportBlock title="Пошаговый план" subtitle="Что делать дальше и какой результат получить" icon={<Route size={19} />}>
        <div className="grid gap-3 lg:grid-cols-2">
          {roadmap.length ? roadmap.map((step, index) => <article key={`${text(step.title)}-${index}`} className="clay-inset rounded-[22px] p-4"><div className="flex items-start gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-signal text-sm font-semibold text-ink">{index + 1}</span><div><h3 className="font-semibold">{text(step.title)}</h3><p className="mt-1 text-xs text-signal">{text(step.stage)}{text(step.authority, "") ? ` · ${text(step.authority)}` : ""}</p><p className="mt-2 text-sm leading-6 text-mist/60">{text(step.description)}</p><p className="mt-3 text-xs text-mist/45">Результат: {text(step.expected_document)}</p></div></div></article>) : <Empty value="План появится после завершения анализа." />}
        </div>
      </ReportBlock>
      ...
      <ReportBlock title="Официальные источники отчёта" subtitle="Документы и ведомства, использованные в выводах" icon={<Database size={19} />}>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {sources.length ? sources.map((source, index) => <div key={String(source.source_key ?? index)} className="clay-inset rounded-[20px] p-4"><p className="font-medium leading-5">{text(source.name)}</p><p className="mt-2 text-xs text-mist/45">{label(source.level)} · {label(source.status)}</p>{typeof source.url === "string" && source.url && <a className="mt-3 inline-flex items-center gap-1 text-xs text-signal" href={source.url} target="_blank" rel="noreferrer">Открыть <ArrowUpRight size={13} /></a>}</div>) : <Empty value="Источники для текущего отчёта ещё собираются." />}
        </div>
      </ReportBlock>
    </section>
  );
