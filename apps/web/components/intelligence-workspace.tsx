...
import {
  ...
  CircleHelp, FileSearch, Gauge, Landmark, MapPinned, Menu, Network,
...
export function IntelligenceWorkspace() {
  useEffect(() => {
    ...
    animate("[data-progress]", { scaleX: [0, 1] }, { duration: 0.75, delay: 0.08 });
  }, []);
      ...
      <div className="workspace-frame">
        <aside className="desktop-sidebar glass-surface">
          ...
          <nav className="space-y-1 text-sm">
            {[
              [Gauge, "Главная"], [BriefcaseBusiness, "Проект"], [Landmark, "Меры поддержки"],
        ...
        <section className="min-w-0 flex-1">
          <header className="topbar glass-surface">
            ...
            <div className="hidden min-w-[280px] max-w-xl flex-1 md:block"><div className="search-shell"><Search size={16} /><span>Поиск мер поддержки, документов, программ...</span></div></div>
          ...
          <div className="bento-grid">
            <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="hero-bento glass-surface">
            ...
            <section className="actions-bento glass-surface">
              <SectionHead eyebrow="Next best actions" title="Следующие действия" icon={<Target size={18} />} />
              <div className="space-y-3">
                <Action index="1" title="Проверить правила конкурса" text="Найти актуальный порядок Минсельхоза КБР." urgent />
                <Action index="2" title="Собрать спрос" text="10–15 интервью и 5–10 писем о намерениях." />
                <Action index="3" title="Подтвердить экономику" text="Получить КП и проверить cash flow." />
              </div>
            </section>
            ...
            <Tabs.Root defaultValue="lenses" className="lenses-bento glass-surface">
              <Tabs.List className="tabs-list" aria-label="Разделы аналитики">
                <Tabs.Trigger className="tab-trigger" value="lenses">Контуры</Tabs.Trigger>
                <Tabs.Trigger className="tab-trigger" value="support">Поддержка</Tabs.Trigger>
                <Tabs.Trigger className="tab-trigger" value="risks">Риски</Tabs.Trigger>
              </Tabs.List>
              <Tabs.Content value="lenses" className="pt-5">
                <div className="mb-5 flex items-end justify-between"><div><p className="eyebrow">14 контуров</p><h3 className="section-title">Карта готовности решения</h3></div><span className="text-xs text-mist">Сейчас</span></div>
            ...
            <section className="blockers-bento glass-surface">
              <SectionHead eyebrow="Decision blockers" title="Критические блокеры" icon={<AlertTriangle size={18} />} />
              <div className="space-y-3">
                <Blocker tone="danger" title="Региональный конкурс КБР" text="Правила конкурса не найдены или требуют обновления." />
                <Blocker tone="danger" title="Финансовая модель" text="Не подтверждены CAPEX, OPEX и денежный поток." />
                <Blocker tone="amber" title="Правовое соответствие" text="Требуется проверка формы КФХ/ООО и ограничений." />
              </div>
            </section>

            <section className="support-bento glass-surface">
              <SectionHead eyebrow="Support matching" title="Подходящие меры поддержки" icon={<Landmark size={18} />} />
              <div className="space-y-3">
                <SupportRow title="Агростартап" source="Минсельхоз РФ" score={67} />
                <SupportRow title="Лизинг сельхозтехники" source="Росагролизинг" score={61} />
                <SupportRow title="Субсидии на технику" source="КБР" score={48} />
              </div>
            </section>

            <section className="evidence-bento glass-surface">
              <SectionHead eyebrow="Evidence locker" title="Доказательства" icon={<BadgeCheck size={18} />} />
              <div className="space-y-3">{evidence.map((item) => <EvidenceRow key={item.title} {...item} />)}</div>
            </section>

            <section className="finance-bento glass-surface">
              <div className="flex items-center gap-3"><div className="icon-tile"><WalletCards size={20} /></div><div><p className="text-sm font-medium">Финансовый контур</p><p className="text-xs text-mist">Критическое покрытие: 38%</p></div></div>
