...
const navigation = [
  ...
  { href: '/control-center/intelligence', label: 'Государственная аналитика', mobileLabel: 'Аналитика', icon: BrainCircuit },
    ...
    <>
      <aside className="desktop-sidebar glass-surface">
        ...
        <div className="mt-7 space-y-1">
          ...
          <div className="nav-item cursor-default opacity-70"><Database size={18} strokeWidth={1.8} /><span className="text-sm">Граф в PostgreSQL</span></div>
          <div className="nav-item cursor-default opacity-70"><ShieldCheck size={18} strokeWidth={1.8} /><span className="text-sm">Доказательства и проверка фактов</span></div>
        ...
        <div className="runtime-card">
          <div className="flex items-center justify-between gap-3">
            <span className="text-[11px] text-mist/45">Аналитическое ядро</span>
            <span className="flex items-center gap-2 text-[11px] font-medium text-signal"><span className="signal-dot" /> Подключено</span>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2"><RuntimeMetric label="Версия" value="0.72" /><RuntimeMetric label="Хранилище" value="Supabase" /></div>
          <p className="mt-3 text-[11px] leading-4 text-mist/35">VER436SIA работает внутри основной платформы и не обходит проверку фактов.</p>
        </div>
      </aside>
      ...
      <nav className="mobile-dock glass-surface" aria-label="Мобильная навигация">
        {mobileNavigation.map((item) => {
          ...
          const active = isActive(item.href)
          return (
            <Link key={item.href} href={item.href} className={`flex min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-[18px] px-1 py-2 text-[10px] transition ${active ? 'dock-active text-mist' : 'text-mist/45'}`}>
