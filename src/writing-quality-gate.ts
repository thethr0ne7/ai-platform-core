...
const throatClearing = [
  /вот что важно/giu,
  /важно отметить/giu,
  /следует отметить/giu,
  /давайте разбер[её]мся/giu,
  /по сути дела/giu,
  /нельзя не отметить/giu,
  /\bhere(?:'s| is) what\b/giu,
  /\bit is important to note\b/giu
];

const binaryContrast = [
  /не\s+[^.!?]{1,80},?\s+(?:а|но)\s+[^.!?]{1,80}/giu,
  /\bnot\s+[^.!?]{1,80},?\s+but\s+[^.!?]{1,80}/giu
];

const vagueDeclaratives = [
  /(?:последствия|выводы|причины|перспективы)\s+(?:значительны|очевидны|структурны|масштабны)/giu,
  /(?:это|данный подход)\s+(?:важно|значимо|эффективно)/giu,
  /\bthe implications are significant\b/giu
];

const passiveMarkers = [
  /(?:было|были|будет|будут)\s+(?:сделано|создано|проведено|реализовано|определено|установлено)/giu,
