...
## Статус исправлений (обновлено 31 июля 2026, после подключения Supabase/Vercel)

Всё P0/P1 из этого аудита закрыто в ветке `claude/full-audit-oi96tj`, кроме одного шага, оставленного человеку:

- **P0.1 закрыт.** Через Supabase MCP подтверждено: `telegram-project-api` реально задеплоена в production (project `hgivyjjethjwswjrvroy`, function id `cc164b88`, v3) — просто никогда не была закоммичена. Код вытянут напрямую из Supabase и добавлен в `supabase/functions/telegram-project-api/`. Проверено при переносе: HMAC-проверка Telegram `initData` реализована верно (алгоритм `WebAppData` secret-key по спецификации Telegram, есть проверка свежести `auth_date` на 24 часа), каждое действие с проектом/документом перепроверяет владение по `telegram_user_id`. Но найден реальный дефект в самой production-функции: она отдавала `Access-Control-Allow-Origin: *` — wildcard CORS, ровно то, что README называет отклонённым legacy-паттерном, и в отличие от всех соседних функций (`catalogue-control`, `evidence-review`, `government-opportunity-api`, `measure-direction-enrichment`), которые используют allowlist origin'ов. При переносе в репозиторий CORS приведён к тому же allowlist-паттерну.
  **Требуется ручной шаг:** закоммиченная версия ещё не задеплоена — в проде продолжает работать старая версия с wildcard CORS, пока кто-то не выполнит `supabase functions deploy telegram-project-api` (project ref `hgivyjjethjwswjrvroy`) из этой ветки/main.
- **P0.2–P1.7 закрыты кодом** (`.gitignore`, SSRF-редиректы в `source-relay`, токен вынесен в `SOURCE_RELAY_TOKEN_HASH`, лок-файл пересобран, мёртвый `lockfile-bootstrap.yml` удалён, ESLint подключён и встроен в CI, `next` обновлён до 15.5.22). `npm run platform:verify` проходит целиком.
  **Требуется ручной шаг:** в Vercel (`ai-platform-core`, team `63-gginner`) нужно добавить `SOURCE_RELAY_TOKEN_HASH=2293a393e8c637c9de4ab67fc882af3ecd2e18222d70ef50495fde2b284a76e5` — это тот же хеш, что раньше был захардкожен, так что действующий plaintext-токен на стороне `evidence-source-processor` продолжит работать без ротации. Инструмент Vercel MCP, доступный в этой сессии, не даёт управлять переменными окружения — это можно сделать только вручную через Dashboard.
- **Побочное наблюдение, не почищено:** в том же Supabase-проекте (`hgivyjjethjwswjrvroy`) висят активные functions, явно не относящиеся к AI Platform Core — `university-ingest`, `university-request`, `search`, `route`, `monitoring-ingest`, `monitoring-preview`, `federal-source-bootstrap`, `coverage`, `telegram-payments`, `telegram-bot-runtime`, `telegram-bot-bootstrap`. Это означает, что production-инфраструктура (БД, service-role ключ) физически общая с как минимум ещё одним-двумя другими продуктами — стоит явно решить, осознанный это выбор или наследие, и стоит ли изолировать AI Platform Core в отдельный Supabase-проект. Вне рамок этого аудита, действие не предпринято.
