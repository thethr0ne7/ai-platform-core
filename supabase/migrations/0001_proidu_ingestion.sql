...
create index if not exists ingestion_jobs_source_status_idx
  on public.ingestion_jobs (source_id, status, scheduled_for desc);

create index if not exists data_versions_namespace_subject_idx
  on public.data_versions (namespace, subject);
...
begin
  ...
  return query
    select false, j.status
    from public.ingestion_jobs j
    where j.idempotency_key = p_idempotency_key;
end;
$$;

revoke all on function public.claim_ingestion_job(text,text,text,text,text,text,timestamptz,integer) from public;
grant execute on function public.claim_ingestion_job(text,text,text,text,text,text,timestamptz,integer) to service_role;
