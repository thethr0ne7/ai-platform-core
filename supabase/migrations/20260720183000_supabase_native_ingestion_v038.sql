...
-- Create or reuse an internal scheduler token. The plaintext is kept only in Vault;
-- the application-visible verifier stores only its SHA-256 digest.
do $$
declare
  v_token text;
begin
...
-- Store the public project URL in Vault so cron configuration contains no mutable endpoint literal.
do $$
begin
  if not exists (
    select 1 from vault.decrypted_secrets where name = 'gi_project_url'
  ) then
...
-- Replace the job deterministically when the migration is replayed.
do $$
declare
  v_job_id bigint;
begin
  ...
  from cron.job
  where jobname = 'official-source-ingestion-v038';
