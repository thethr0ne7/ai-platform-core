...
as $$
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'status', case
      ...
      when documents_count > 0 and technical_status in ('degraded','blocked') then 'Данные сохранены, обновление задержано'
      when technical_status = 'blocked' then 'Доступ ограничен'
...
revoke all on function public.gi_get_source_catalog_for_report() from public, anon, authenticated;
grant execute on function public.gi_get_source_catalog_for_report() to service_role, postgres;
