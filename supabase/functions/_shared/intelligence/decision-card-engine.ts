...
export function buildDecisionCards(
  ...
  for (const measure of asRecords(context.report.measure_matches)) {
    ...
    const verifiedEvidence = requirements.filter((requirement) =>
      text(requirement.evidence_status) === "verified" &&
      (text(requirement.evidence_id) || text(asRecord(requirement.actual).evidence_id) || text(requirement.source_quote))
    );
    const verifiedEvidenceCount = verifiedEvidence.length;
    const status = eligibilityStatus(measure.eligibility_status);
    const truthGatePassed = status === "match" && Boolean(evidenceScope.fully_verified) && verifiedRequirementCount > 0 && verifiedEvidenceCount > 0;
    ...
    cards.push({
      ...
      confirmedConditions: requirements
        .filter((requirement) => text(requirement.status) === "matched" && text(requirement.evidence_status) === "verified")
